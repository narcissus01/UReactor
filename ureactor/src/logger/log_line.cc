...
#include<ureactor/logger/log.h>
#include<ureactor/utils/system_utils.h>
...
struct LogLine::Impl final
{
    ...
    InlineBuffer<LOG_MASSAGE_INLINE_CAPACITY> m_inlineBuffer;
    SmallStreamBuffer<LOG_MASSAGE_INLINE_CAPACITY> m_streamBuffer;
...
LogLine::LogLine(Logger& logger, Loglevel::Level level, u32 line, std::string_view file_name)
{
    ...
    std::construct_at(reinterpret_cast<Impl*>(m_implStorage), logger, level, line, file_name); //placement new 技术
}
...
void LogPrintf(Logger& logger, Loglevel::Level level, u32 line, std::string_view file_name, const char* format, ...)
{
    ...
    try
    {
        overflow_buffer.resize(message_size + 1);
    }
    catch (...)
    {
        // resize 分配内存失败时，也必须结束之前 va_copy 的参数列表。
        va_end(arguments_copy);
        throw;  // 清理后，把原异常继续交给调用方。
    }
