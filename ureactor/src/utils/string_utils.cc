...
std::string StringUtils::WstringToString(std::wstring_view text) noexcept
{
    try
    {
        ...
        for (std::size_t i = 0; i < text.size(); ++i)
        {
            ...
            if constexpr (sizeof(wchar_t) == 2)
            {
                ...
                {
                    ++i;
                    ASSERT_RETVAL_MSG(i < text.size(), std::string{}, "宽字符串包含不完整的 UTF-16 代理对");
                    const u32 low_surrogate = static_cast<u32>(text[i]);
                    ASSERT_RETVAL_MSG(IsLowSurrogate(low_surrogate), std::string{}, "宽字符串包含非法 UTF-16 代理对");
                    code_point = 0x10000U + ((code_point - HIGH_SURROGATE_FIRST) << 10U) + (low_surrogate - LOW_SURROGATE_FIRST);
                }
                else
                {
                    ASSERT_RETVAL_MSG(!IsLowSurrogate(code_point), std::string{}, "宽字符串包含孤立的 UTF-16 低代理项");
                }
            }
            else
            {
                ASSERT_RETVAL_MSG(code_point <= MAX_UNICODE_CODE_POINT && !IsHighSurrogate(code_point) && !IsLowSurrogate(code_point),
                               std::string{}, "宽字符串包含非法 UTF-32 码点");
            }
    ...
    {
        ASSERT_NOEFFECT_MSG(false, "宽字符串转换为 UTF-8 时发生异常");
        return {};
    }
}
...
std::wstring StringUtils::StringToWstring(std::string_view text) noexcept
{
    try
    {
        ...
        while (offset < text.size())
        {
            ...
            ASSERT_RETVAL_MSG(decoded, std::wstring{}, "字符串包含非法 UTF-8 编码");
            if constexpr (sizeof(wchar_t) == 2)
            {
    ...
    {
        ASSERT_NOEFFECT_MSG(false, "UTF-8 转换为宽字符串时发生异常");
        return {};
    }
}
